...
This is a step-by-step tutorial to use Monte Carlo simulation for estimating the required sample size for a hypothesis test. Accompanying slides: https://docs.google.com/presentation/d/1wQGDNUw7c2zxIZOnrCpifQcsjeKOdqnVQkeankOBbhA
...
rt_df %>% 
  ...
  geom_boxplot(alpha = .5, width = .5, outlier.alpha = 0) +
...
See: <https://debruine.github.io/faux/articles/distributions.html> o generally we can first generate the datasets with normal variables, and transform the variables into differently distributed variables.
...
lmer(rt ~ group * measurement + age + (1|id), data = wide2long(study2)) %>% 
  summary()
...
Answer: Statistical power is reached when we have 58 participants / cell
which is 232 participants altogether.
